...
<script>
	...
	export default {
		...
		computed: {
			...
			left: ${this.isMobile?clipX:(375 - this.width / 2) / 2}px;
			top: ${clipY}px
			`
			},
			imageStyle() {
				...
				if (this.isMobile) {
					var number = imageLeft - imageWidth / 2
				} else {
					var number = (375 - imageWidth) / 2
				}
				return `
				width: ${imageWidth ? imageWidth + 'px' : 'auto'};
				height: ${imageHeight ? imageHeight + 'px' : 'auto'};
				transform: translate3d(${number}px, ${imageTop - imageHeight / 2}px, 0) scale(${scale}) rotate(${angle}deg);
